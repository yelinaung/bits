...
munit_case(RUN, test_colors_defined, {
  assert_int(RED2, ==, 55, "RED is defined as 55 (nvim green!)");
  assert_int(GREEN2, ==, 176, "GREEN is defined as 176 (nvim green!)");
  assert_int(BLUE2, ==, 38, "BLUE is defined as 38 (nvim green!)");
});

munit_case(SUBMIT, test_colors_defined_correctly, {
  assert_int(RED2, !=, 0, "RED is not defined as 0 (vsc*de blue!)");
  assert_int(GREEN2, !=, 120, "GREEN is not defined as 120 (vsc*de blue!)");
  assert_int(BLUE2, !=, 215, "BLUE is not defined as 215 (vsc*de blue!)");
});
